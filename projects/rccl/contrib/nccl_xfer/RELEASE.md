...
| **Internal caches are lock-guarded; single-process/multi-rank supported** | The internal caches (DevComm cache, window cache, stream pool) are process-wide shared state guarded by an internal recursive mutex, so rank threads that each drive their own `ncclComm_t` in one process may call `ncclXferReshardWithWindow` concurrently (as in the single-process/multi-rank tests). The DevComm cache is keyed on `(comm, numCtas, signalCount)` — not on the CUDA stream — so all ranks make the same create-vs-reuse decision for the collective `ncclDevCommCreate`. `ncclXferReshardInit` / `ncclXferReshardFinalize` are still expected to be called once per process at start/teardown, not concurrently. Device-side concurrency (successive reshards on separate CUDA streams from one host thread) remains supported. |
