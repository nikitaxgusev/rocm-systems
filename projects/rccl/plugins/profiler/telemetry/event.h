...
struct collective {
  ...
  // Per-collective RCCL network telemetry, accumulated EXACTLY from the child
  // NetPlugin (per-QP IB WQE) events of this collective. Each NetPlugin event is
  // attributed to exactly one collective via the parent chain
  // NetPlugin -> proxyStep -> proxyOp -> collective, so this is correct even when
  // collectives pipeline/overlap (unlike subtracting global device counters).
  // Direction (tx vs rx) comes from proxyOp->isSend.
  int      telValid;     // set once >=1 net event was attributed to this coll
  uint64_t telTxBytes;   // sum of WQE lengths on send QPs
