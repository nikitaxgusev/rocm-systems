 ...
 *
 * Upstream NCCL PR #1759 ("Use poll to avoid busy loops for bootstrap
 * socket-io") added the NCCL_SOCKET_POLL_TIMEOUT_MSEC parameter. When set to a
 * non-zero value, socketWait() blocks in poll() until the socket becomes
 * readable/writable instead of spinning on socketProgress(), which previously
 * pinned a CPU core at ~100% for the whole duration of a bootstrap transfer
 * that was waiting on a slow / late peer.
 *
...
void CheckSlowTransfer(bool expectBusy) {
  ...
  if (expectBusy) {
    ...
    EXPECT_LT(ratio, kCpuBusyThreshold)
        ...
        << "socket poll fix (NCCL PR #1759) is missing or ineffective. cpu="
