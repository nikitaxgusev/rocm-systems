  ...
  *eHandle = NULL;
  ...
  if (eDescr->type == ncclProfileGroupApi) {
    ...
    *eHandle = event;
    ...
    // Per-collective network telemetry is accumulated from this collective's
    // child NetPlugin events (see ncclProfileNetPlugin handler). Zero the
    // accumulators here; they fill in as QP WQE events arrive.
    event->telValid = 0;
    event->telTxBytes = 0;
    event->telRxBytes = 0;
    event->telWqeSent = 0;
    ...
    if (type == NCCL_PROFILER_NET_TYPE_IB) {
      if (ver == 1) {
        ...
        if (descr->type == ncclProfileQp) {
          ...
          // Attribute this QP WQE to its owning collective via the parent chain
          // NetPlugin -> proxyStep(parent) -> proxyOp(parent) -> collective.
          // Direction comes from proxyOp->isSend. This is exact under pipelining.
          struct proxyOp* pop = parent->parent;
          if (pop && pop->parent && pop->parent->type == ncclProfileColl) {
            struct collective* coll = (struct collective *)pop->parent;
            if (pop->isSend) {
              __atomic_fetch_add(&coll->telTxBytes, (uint64_t)descr->qp.length, __ATOMIC_RELAXED);
              __atomic_fetch_add(&coll->telWqeSent, 1, __ATOMIC_RELAXED);
            } else {
              __atomic_fetch_add(&coll->telRxBytes, (uint64_t)descr->qp.length, __ATOMIC_RELAXED);
              __atomic_fetch_add(&coll->telWqeRcvd, 1, __ATOMIC_RELAXED);
            }
            coll->telValid = 1;
          }
        }
